      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex flex-col items-center space-y-4">
          <div class="relative w-24 h-24">
            ...
            <label class="absolute bottom-0 right-0 bg-blue-600 rounded-full p-2 text-white cursor-pointer hover:bg-blue-700 transition shadow-sm">
        ...
        <div>
          ...
          <input 
            ...
            title="핸들은 @로 시작하고 영문, 숫자, _, .만 사용할 수 있습니다 (6~20자)"
            @input="onHandleInput"
          />
          <p v-if="handleMessage" :class="handleMessageClass" class="text-sm mt-1">{{ handleMessage }}</p>
        </div>
...
const handleMessageClass = computed(() => {
  return isHandleAvailable.value ? 'text-green-500' : 'text-red-500'
})
...
onMounted(() => {
  if (authStore.user) {
    ...
    form.value.bio = (authStore.user as any).bio || ''
...
const onHandleInput = () => {
  ...
  if (!form.value.handle || !/^@[a-zA-Z0-9_.]{6,20}$/.test(form.value.handle)) {
    handleMessage.value = '핸들은 @로 시작하고 영문, 숫자, _, .만 사용할 수 있습니다 (6~20자)'
  ...
  handleTimeout = setTimeout(async () => {
    try {
      ...
      handleMessage.value = result.available ? '사용 가능한 핸들입니다.' : '이미 사용 중인 핸들입니다.'
    } catch (e) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data || '프로필 수정에 실패했습니다.'
  } finally {
